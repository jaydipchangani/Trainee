...
const DraggableRegion: React.FC<{
  ...
  return (
    <div
      ...
      onClick={
        isInMappingColumn ? undefined : () => onMapped && onMapped(region.id)
      }
    >
...
const MapGroup: React.FC<MapGroupProps> = ({ title }) => {
  ...
  const [selectedClassValue, setSelectedClassValue] = useState<string | null>(
    null
  );
  const [showMappedRegionsInFirstColumn, setShowMappedRegionsInFirstColumn] =
    useState(true);
  ...
  const handleCancel = () => {
    setIsModalOpen(false); // Close modal // Clear form data
    navigate(-1); // Go to the last page
  };
  ...
  const handleDrop = (classValueId: string, regionId: string) => {
    ...
    setMappings((prevMappings) => {
      ...
      const existingMappingIndex = newMappings.findIndex(
        (m) => m.classValueId === classValueId
      );
  ...
  const handleRemoveMapping = (classValueId: string, regionId: string) => {
    ...
    newMappings[mappingIndex].regionIds = newMappings[
      mappingIndex
  ...
  const getMappedRegionsForClassValue = (classValueId: string) => {
    ...
    return mapping.regionIds
      .map(
        (regionId) =>
          availableRegions.find((r) => r.id === regionId) ||
  ...
  const handleUpdate = async () => {
    try {
      ...
      console.log(
        "Group ID (Using selectedGroupClassId):",
        localStorage.getItem("selectedGroupClassId")
      );
      console.log(
        "Group Class ID:",
        localStorage.getItem("selectedGroupClassId")
      ...
      if (!token || groupId === "0" || groupClassId === "0") {
        console.log(
          "Authentication token or required IDs are missing! Please check your login or selection."
        );
        return;
      }
      ...
      localStorage.setItem(
        "mappedData",
        JSON.stringify(payload.gRCValuesDetails)
      );
      console.log("Mapped data stored in localStorage:");
    ...
    return (
      ...
      >
        <div style={{ display: "flex", gap: "24px", height: "100%" }}>
          {/* Left Column */}
          <Card
            title="Class Values & Mapped Regions"
            style={{ flex: 1, overflow: "auto" }}
          >
            {mappings.map((mapping) => {
              const classValue = classValues.find(
                (cv) => cv.id === mapping.classValueId
              );
          ...
          <Card title="Available Regions" style={{ flex: 1, overflow: "auto" }}>
            <ul>
              {regions
                .filter(
                  (region) =>
                    !mappings.some((m) => m.regionIds.includes(region.id))
                )
                .map((region) => (
                  <li key={region.id}>{region.name}</li>
                ))}
      ...
      <Layout style={{ background: "white", height: "100vh" }}>
        ...
        <Content style={{ padding: "16px" }}>
          ...
          >
            ...
            >
              {classValues.map((classValue) => {
                const mappedRegions = getMappedRegionsForClassValue(
                  classValue.id
                );
              ...
              {mappings
                ...
                .map((mapping) => (
                  ...
                  >
                    <strong>
                      {
                        classValues.find((cv) => cv.id === mapping.classValueId)
                          ?.name
                      }
                    </strong>
                    {mapping.regionIds.map((regionId) => {
            ...
            >
              {availableRegions
                .filter(
                  (region) =>
                    !mappings.some((m) => m.regionIds.includes(region.id))
                ) // Remove mapped regions
                .map((region, index) => (
                  <div
                    key={region.id}
                    id={`available-region-${index}`}
                    style={{
                      padding: "8px",
                      borderBottom: "1px solid #f0f0f0",
                    }}
          ...
          <div style={{ marginTop: "16px", marginBottom: "16px" }}>
            <Button onClick={handleToggleView} style={{ marginRight: "16px" }}>
              {showMappedRegionsInFirstColumn
                ? "Show mappings in middle column"
                : "Show mappings in first column"}
            </Button>
          </div>
