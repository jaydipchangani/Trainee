...
const MultiEntityDisplay: React.FC = () => {
  ...
  const fetchGroupClassData = async () => {
    ...
    try {
      ...
      setGroupClassData(
        data.result.records.map((item: any, index: number) => ({
          key: String(index + 1),
          ...
          setupOrMapping: (
            <Link
              to={`/configurationDisplay?groupclassId=${item.groupclassId}`}
            >
              Mapped: {item.mappedAccountsCount}, Unmapped:{" "}
              {item.unMappedAccountsCount}
            </Link>
          ),
        }))
      );
    ...
    <Layout className="entity-display-layout">
      ...
      <Content className="content">
        ...
        <div className="tab-container">
          ...
          >
            <Menu.Item key="company" onClick={() => setActiveTab("company")}>
              Company
            </Menu.Item>
            <Menu.Item key="group" onClick={() => setActiveTab("group")}>
              Group
            </Menu.Item>
            <Menu.Item
              key="groupClass"
            ...
            <Menu.Item
              ...
              onClick={() => setActiveTab("configurationDisplay")}
            >
          ...
          <div className="actions-container">
            ...
            {activeTab === "group" && (
              <Button
                ...
                icon={<PlusOutlined />}
                className="add-button"
                onClick={() => setGroupDrawerVisible(true)}
              >
                Add Group
              </Button>
            )}
        ...
        <div className="table-container">
          {activeTab === "groupClass" ? (
            loading ? (
              ...
              <GroupClassTable
                data={filteredGroupClassData}
                setData={setGroupClassData}
              />
            )
          ) : activeTab === "group" ? (
            loading ? (
